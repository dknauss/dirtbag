...
- **A small custom-CSS block.** `theme.json` → `styles.css` is the theme's one sanctioned home for hand-written CSS — the deliberate exception, for rules core settings cannot express declaratively. Keep it short and justified; every rule here ships inline on every page. It currently carries four:
  ...
  4. The `.sidebar-thumb` grid. The front-page sidebar posts show a small square thumbnail with the title/date/excerpt beside it. `theme.json`'s structured layout can't express a fixed-width media column over the flat `post-template` output, so a CSS grid gives `.sidebar-entry` a `60px 1fr` two-track layout: the thumbnail occupies the fixed first track (spanning the text rows so it doesn't inflate the title's row), title/date/excerpt stack in the second, clipped square with `overflow: hidden`. This ships **instead of** a `float` that wrapped text *under* the thumb (the magazine look) because the float collided with WordPress core's Post Title link CSS: `.wp-block-post-title :where(a) { display: inline-block; }`. That atomic inline-block title drops below a 60px float whenever it cannot fit in the remaining line box. It is **not** lazy-loading, sub-pixel DPR, or the `.front-grid` subgrid; a captured live-page reduction is fixed by overriding the title link to `display:inline` in the float variant. A grid has no float to break. `.sidebar-head` shrinks the sidebar section heading to 22px so it fits one line in the narrow column, and `align-self: end` baselines it against the taller wide-column heading. The float is kept as a one-block drop-in alternative, with the full root-cause writeup and reduction notes, in [sidebar-thumbnail-layout.md](sidebar-thumbnail-layout.md).
