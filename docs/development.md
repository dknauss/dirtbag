...
- **A small custom-CSS block.** `theme.json` → `styles.css` is the theme's one sanctioned home for hand-written CSS — the deliberate exception, for rules core settings cannot express declaratively. Keep it short and justified; every rule here ships inline on every page. It currently carries four:
  ...
  4. The `.sidebar-thumb` float. The front-page sidebar posts use a small square thumbnail that the title/date/excerpt wrap around and under — a magazine float the `post-featured-image` block can't express (it offers a fixed flex size that doesn't scale and rendered non-square). A CSS rule floats the figure left at a responsive `clamp(36px, 5vw, 44px)` square (`aspect-ratio: 1` on the image), and `.sidebar-entry { display: flow-root }` contains each entry's float so they clear cleanly. The same rule zeroes the post title's top margin (so it aligns with the thumbnail's top rather than dropping below it) and sizes it to 20px, so each entry reads as a compact secondary feed beside the main grid. Keeps the sidebar thumbs well below the main grid images at every width.
