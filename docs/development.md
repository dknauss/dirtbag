...
- **A small custom-CSS block.** `theme.json` → `styles.css` is the theme's one sanctioned home for hand-written CSS — the deliberate exception, for rules core settings cannot express declaratively. Keep it short and justified; every rule here ships inline on every page. It currently carries four:
  ...
  4. The `.sidebar-thumb` float. The front-page sidebar posts use a small square thumbnail that the title/date/excerpt wrap around and under — a magazine float the `post-featured-image` block can't express. A CSS rule floats the figure left at a responsive `clamp(48px, 6vw, 60px)` **fixed square with `overflow: hidden`**, with the link+image absolutely positioned out of the box's flow so the floated box keeps its size regardless of the (lazy) image's load state. `.sidebar-entry { display: flow-root }` contains each entry's float; the same rule zeroes the post title's top margin and sizes it to 20px so each entry reads as a compact secondary feed. The wrap-under is shipped deliberately despite one known Chrome glitch — below-the-fold entries can stack the title under the thumb while the lazy image is still unloaded, self-healing once it loads; the only clean fix (`loading="eager"`) needs a `functions.php` filter the theme forbids, and a no-float CSS-grid fallback is documented but trades away the wrap-under. `.sidebar-head` shrinks the sidebar section heading to 22px so it fits one line in the narrow column, and `align-self: end` baselines it against the taller wide-column heading. See [sidebar-thumbnail-layout.md](sidebar-thumbnail-layout.md) for the full root-cause writeup and the grid fallback.
